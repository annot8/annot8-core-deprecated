 ...
 *
 * You have an Item, which is your container and holds information about the data you wish to
 ...
 *
 * On to these Content objects, you create Annotations. Annotations indicate that a certain part of
 * the Content has some special meaning (e.g. it is a mention of a person). These Annotations are
 ...
 *
 * You can link Annotations together with Groups. When adding an Annotation to a Group, you must
 ...
 *
 * Most of the objects above support Properties, which allow you to attach additional information to
 * an object. Many objects also have a type and an ID.
 *
 * There are three types of 'component' which are used to create and manipulate the above objects. A
 * Source will create new Items from some external source, for example a file system or a database.
 * The Items created by a Source will be passed to one or more Processors, which will manipulate the
 * Items by adding new Content objects to it, manipulating the Annotations within these Content
 * objects, and creating Groups of Annotations.
 *
 * Both the Sources and Processors can use the third type of component - a Resource - to access
 * common or external resources (e.g. a database connection, or reference dataset).
 *
 * It is envisaged that Sources, Processors and Resources will generally be corralled together to
